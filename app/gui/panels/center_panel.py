...
class CenterPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        # View buttons - без QButtonGroup, управляем состоянием вручную
        view_labels = ['RGB', 'Маска', 'Overlay', 'Контуры']
        ...
        self.btn_fit.clicked.connect(lambda: self.viewer.fit())
    ...
    def _apply_enhancement(self, _=None):
        ...
        contrast   = 1.0 + self.sl_contrast.value() / 100.0   # -50..150 → 0.5..2.5
        sharpness  = self.sl_sharpness.value() / 100.0 * 3.0  # 0..100 → 0..3.0
