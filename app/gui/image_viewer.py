...
class ImageViewer(QGraphicsView):
    def __init__(self, parent=None):
        ...
        self._orig_shape: tuple[int, int] | None = None  # (h, w) original before downscale
    ...
    def set_image(self, arr: np.ndarray):
        """float32 [0..1] или uint8, (H,W) или (H,W,3)"""
        self._orig_shape = arr.shape[:2]
        self._base_arr_u8 = _to_uint8(arr)
        self.clear_highlight()
        self._apply_and_show(reset_zoom=True)
    ...
    def highlight_object(self, contour, orig_shape: tuple[int, int]):
        ...
        # Приближаем к объекту с паддингом ~40% от его размера
        pad_x = max(rw * 0.4, 20)
        pad_y = max(rh * 0.4, 20)
